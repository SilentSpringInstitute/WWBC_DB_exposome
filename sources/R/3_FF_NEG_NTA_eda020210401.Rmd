...
Step 3.
Meets FB filter and 
Ubiquitous breast cancer relevant
...
```{r, echo=FALSE }
#read in data. 
neg_xset <- read.csv("~/Box/SHE lab/WWBC/Data analysis/Non-targeted analysis/FF_non-targeted/ESI_neg/preprocessing_neg/20210423neg_xset.csv") 
neg_xset <- neg_xset[ , c(2,5,10:137)]
# colnames(neg_xset)

#make feature_id variable and reorganize columns
neg_xset$mz<-round(neg_xset$mz,5)
neg_xset$rt <- round(neg_xset$rt, 1)
neg_xset$featureid<-as.character(paste(neg_xset$mz,neg_xset$rt,sep = "-"))
dataB_neg_xset<-neg_xset[,-131]
...
# dtsc <- dtsc[ ,-1 ]
# positions_dtsc = seq(1,124, by = 2) #because each run is repeated so select first of the 2 runs
# dtsc <- dtsc[ positions_dtsc,]
dtsc$dtsc_fid <- as.character(paste(dtsc$dtsc_id,dtsc$dtsc_run,sep = ".")) #make a sample ID to match features data. 
...
#Renumber the 2 participatns who gave samples for 2 fires and assign the visits 1,2,3 so they are included in the analysis. 
dtsc <- dtsc%>% mutate(new_visit= ifelse(FF_id == "F227" & visit == "Visit 3", "Visit 1", 
...
new_df_long <- gather(neg_xset, "sample_id", "intensity", BD01346.r001:DIWB2.B7 )
#keep relevant vars
dtsc <- dtsc[, c("dtsc_id","dtsc_fid", "visit", "FF_id", "new_visit", "newFF_id")]
...
# table(dtsc$new_visit)

#regarding the use of 0 versus NA. WE decided today 8/28/2020 that we would substitute 0 for the non-detects in calculating the summary statistics that are used to select the non-matched features for fragmentation. We will also use the median to compare difference between visit 1, 2 and 3 (looking at the absolute difference of intensity and detection frequency of the visits) and select the 10 features with the largest absolute difference for each mode and for visit 1 vs 2 and 1 vs 3 N= 80?
...
annotation_neg <- read.csv(file = "/Users/trowbridgej/Box/SHE lab/WWBC/Data analysis/Non-targeted analysis/FF_non-targeted/ESI_neg/Neg_annotator_20210708/Stage5.csv")

#make the feature id variable on which to merge. 
colnames(annotation_neg)[6] <- "rt"
annotation_neg$mz<-round(annotation_neg$mz,5)
annotation_neg$rt <- round(annotation_neg$rt, 1)
annotation_neg$featureid<-as.character(paste(annotation_neg$mz,annotation_neg$rt,sep = "-"))

#keep relevant variables
annotation_neg<-annotation_neg[,c("featureid","chemical_ID","Confidence","score","mz","rt","MatchCategory","theoretical.mz", "delta_ppm","Name","Formula","MonoisotopicMass","Adduct")]

#rename to match previous datasets
names(annotation_neg)[names(annotation_neg)=="chemical_ID"] <- "DTXSID"
names(annotation_neg)[names(annotation_neg)=="Name"]<-"Compound"

#FILTER/Remove confidence <1. 

annotation_neg <- annotation_neg%>% 
  filter(Confidence >0)
...
df_annotation<-merge(annotation_neg, ssi_databse, by= "DTXSID", all= TRUE)
